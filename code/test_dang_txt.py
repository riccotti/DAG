...
def main():
    ...
    D = get_dataset(dataset, path_dataset, categories=['alt.atheism', 'talk.religion.misc'])
    ...
    if data_type == 'tab':
        ...
        window_shape = 3
        step = 3
        X_train_dt = [TextRecord(x, window_shape=window_shape, step=step, text_length=text_length) for x in X_train]
        x_dt = TextRecord(X_test[idx], window_shape=window_shape, step=step, text_length=text_length)
